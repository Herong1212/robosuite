...
def make(env_name, *args, **kwargs):
    ...
    """
    # 首先检查传入的环境名称是否存在于已注册的环境字典中
    if env_name not in REGISTERED_ENVS:
        # 如果环境名称不存在于注册环境中，则抛出异常，提示无效的环境名称
        raise Exception(
    ...
    # 如果环境名称有效，则从注册环境字典中取出对应的环境类，并使用传入的参数进行实例化
    # 返回值不是字典，而是一个 MujocoEnv 类型的对象，具体是 REGISTERED_ENVS[env_name] 所指向的环境类的实例
